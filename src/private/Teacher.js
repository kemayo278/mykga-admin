    ...
    const fetchTeacher = async () => {
        ...
        try {
          const unsubscribe = onSnapshot(
            query(collection(db, "users"), where("user_type", "==", "Teacher")),
            (snapShot) => {
              let list = [];
      ...
      if (result.isConfirmed) {
        if(user_state !== "idle")
        {
          try {
              ...
              Swal.fire({position: 'top-right',icon: 'success',title: 'Thanks you!',text: 'This Teacher has been deactivated',showConfirmButton: true})
          } catch (error) {
            Swal.fire({position: 'top-right',icon: 'error',title: 'Oops!',text: 'An error occurred while executing the program',showConfirmButton: true,confirmButtonColor: '#3085d6',})
          }
        }
        else{
          Swal.fire({position: 'top-right',icon: 'warning',title: 'Information',text: 'This Teacher is already deactivated',showConfirmButton: true,confirmButtonColor: '#3085d6',})
        }
      }
    });
  };

  const ChangeStateTeacherToEnable  = async(id,user_state) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if(user_state !== "asset")
        {
          try {
              ...
              Swal.fire({position: 'top-right',icon: 'success',title: 'Thanks you!',text: 'This Teacher has been activated',showConfirmButton: true})
          } catch (error) {
            Swal.fire({position: 'top-right',icon: 'error',title: 'Oops!',text: 'An error occurred while executing the program',showConfirmButton: true,confirmButtonColor: '#3085d6',})
          }
        }
        else{
          Swal.fire({position: 'top-right',icon: 'warning',title: 'Information',text: 'This Teacher is already activated',showConfirmButton: true,confirmButtonColor: '#3085d6',})
        }
      }
    });
  };  
                  ...
                  <table id="example" class="table table-condensed">
                    ...
                    <>
                      {teachers && teachers.map((teacher) => {
                        ...
                        else if (teacher.user_state === "idle") { classState = "bg-danger text-white";contentState = "Disable";}                      
                        return (
                          <tr>
                              <td className="vertical-align-middle">
                                {teacher.user_img != null ? (
                                  <Link to={teacher.user_img}>
                                    <img
                                      src={teacher.user_img}
                              ...
                              <td className="vertical-align-middle">{teacher.user_fulname}</td>
                              <td className="vertical-align-middle">{teacher.user_email}</td>
                              <td className="vertical-align-middle"><span className={`badge rounded-pill ${classState}`} style={{ padding: "10px" }} >{contentState}</span></td>
                              <td className="vertical-align-middle">{teacher.user_phone}</td>
                              <td className="vertical-align-middle">{teacher.user_address}</td>
                              <td className="vertical-align-middle">
                                  <a onClick={() => ChangeStateTeacherToEnable(teacher.user_id,teacher.user_state)} className="btn btn-outline-info" style={{ borderRadius: "5px", padding: "5px" }}>Enable</a>
                              </td>
                              <td className="vertical-align-middle">
                                  <a onClick={() => ChangeStateTeacherToDisable(teacher.user_id,teacher.user_state)} className="btn btn-outline-danger" style={{ borderRadius: "5px", padding: "5px" }}>Disable</a>
                              </td>
                          </tr>     
                          ); 
                        })}
